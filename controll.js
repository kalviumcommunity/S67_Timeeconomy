...
router.post('/create', async (req, res) => {
  try {
    ...
    const newDataItem = await dataItem.create({ name, description, price });
    res.status(201).json({
      success: true,
...
router.put('/update/:id', async (req, res) => {
  try {
    ...
    const updatedDataItem = await dataItem.findByIdAndUpdate(
      req.params.id,
...
router.delete('/del/:id', async (req, res) => {
  try {
    const daleteDataItems = await dataItem.findByIdAndDelete(req.params.id);
    if (!daleteDataItems) {
      return res.status(404).json({ success: false, message: 'Data item not found' });
    }
